...
class ComponentDataset(Dataset):
    def __init__(self, df, is_training=False):
        ...
        if is_training:
            ...
            for _, row in positives.iterrows():
                ...
                # Only keep if long enough to have a "middle" to swap
                if len(i1) > 100:
                    self.positive_intron1_middles.append(i1[50:-50])
                if len(i2) > 100:
                    self.positive_intron2_middles.append(i2[50:-50])
            
            print(f"Franken-Bank Loaded: {len(self.positive_intron1_middles)} I1 parts, {len(self.positive_intron2_middles)} I2 parts.")
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training:
            ...
            # --- AUGMENTATION 1: FRANKEN-INTRON SWAP ---
            if label == 1.0:
                # Swap Intron 1 Middle (50% chance)
                ...
                # Swap Intron 2 Middle (50% chance)
            ...
            # --- AUGMENTATION 3: ADVERSARIAL DECOYS ---
            if label == 1.0:
                rand_val = np.random.rand()
                # 15% chance to destroy GT
                if rand_val < 0.15:
                     i2 = "CC" + i2[2:] 
                     label = 0.0 
                # 15% chance to scramble content
                elif rand_val < 0.30:
...
def train_model():
    log_message(f"--- Starting Hybrid V11 (FRANKEN) Training on {torch.cuda.get_device_name(0)} ---")
    log_message(f"Augmentations: Intron Swapping (New!), Jitter, Mutation, Shuffling, Masking")
