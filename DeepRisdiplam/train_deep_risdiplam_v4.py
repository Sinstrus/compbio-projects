...
EPOCHS = 50
MAX_SEQ_LEN = 1500 
LEARNING_RATE = 0.0001  # Ultra-conservative start (was 0.0005)
STEPS_PER_EPOCH = 100 
NUM_WORKERS = 8 
...
class BalancedIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        while True:
            ...
            y[:self.n_pos] = 1.0 # Will be smoothed in loop
            yield X, y

class RisdiplamModel(nn.Module):
    def __init__(self):
        super(RisdiplamModel, self).__init__()
        # Same architecture, it works well enough
        ...
        self.dropout = nn.Dropout(0.6) # Increased Dropout (was 0.5)
...
def evaluate(model, val_loader, steps=20):
    ...
    # Track average prediction to check for collapse
    avg_pred = 0.0
...
def main():
    ...
    log_message(f"Starting V4 Training (Label Smoothing + Low LR). Device: {device}")
    ...
    criterion = nn.BCEWithLogitsLoss()
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-3) # Higher weight decay
    ...
    for epoch in range(EPOCHS):
        ...
        for _ in pbar:
            ...
            # --- LABEL SMOOTHING ---
            # Instead of 0 and 1, we target 0.05 and 0.95
            # This prevents the model from being "over-confident" and causing infinite loss spikes
        ...
        val_loss, val_acc, val_avg_pred = evaluate(model, val_loader)
        
        scheduler.step(val_acc)
        
        log_msg = f"Epoch {epoch+1}: Loss={val_loss:.4f} | Acc={val_acc:.4f} | AvgPred={val_avg_pred:.2f}"
        log_message(log_msg)
