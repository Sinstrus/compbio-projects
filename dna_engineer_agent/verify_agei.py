...
print("="*80)
print("VERIFYING AgeI @ 3583 FOR REGION 3")
print("="*80)
...
if Seq(original_codon).translate() == Seq(mutated_codon).translate():
    print(f"\n✅ SILENT - Both encode {Seq(original_codon).translate()}")
    print(f"\n{'='*80}")
    print("RECOMMENDATION: AgeI @ 3583")
    print(f"{'='*80}")
    print("AgeI is an excellent choice:")
    print("  ✓ Very common, reliable enzyme")
    print("  ✓ Works well in most buffers")
    print("  ✓ Widely available")
    print("  ✓ Compatible with many cloning workflows")
    print("  ✓ Generates 5' overhang (good for directional cloning)")
else:
    print(f"\n❌ NOT SILENT - Changes {Seq(original_codon).translate()} → {Seq(mutated_codon).translate()}")
