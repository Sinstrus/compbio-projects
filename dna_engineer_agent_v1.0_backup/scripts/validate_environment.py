...
def list_cis_elements(base_path: Path):
    ...
    try:
        ...
        print(f"\n  {len(elements)} elements loaded:\n")
        
        for elem in elements:
            elem_id = elem.get("id", "?")
            elem_type = elem.get("type", "?")
            risk = elem.get("risk_level", "?")
            method = elem.get("detection_method", "?")
            ...
            print(f"  {risk_symbol} [{risk:8}] {elem_id}")
            print(f"             Type: {elem_type}, Detection: {method}")
...
def main():
    ...
    # Determine base path (script location or current directory)
