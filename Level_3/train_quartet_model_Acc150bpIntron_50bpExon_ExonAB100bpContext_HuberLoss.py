...
# --- 1. The Deep Context Architecture ---
class QuartetNet(nn.Module):
    def __init__(self):
        super(QuartetNet, self).__init__()
        
        # Branch A/B: Flanking Contexts
        # Wider window (100bp) to see context strength
        ...
        # Branch C: Pseudoexon Acceptor (Deep Intron View)
        # Input: 200bp (150bp Intron + 50bp Exon)
        # Captures Branch Point and Distant ISEs
        ...
        # Branch D: Pseudoexon Donor (Drug Target)
        # Input: 200bp (50bp Exon + 150bp Intron)
        # Captures Drug Motif + Downstream Context
        ...
        # Fusion
        # Contexts: 25 * 64 = 1600 features each
        # Targets: 50 * 128 = 6400 features each
        total_feats = 1600 + 1600 + 6400 + 6400 
        
        self.fc_fusion = nn.Sequential(
            nn.Linear(total_feats, 1024),
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(1024, 256),
            nn.ReLU(),
...
def get_real_sample(row):
    try:
        # Widen Flanking Contexts to 100bp
        # Ctx A: Last 50 Exon A + First 50 Intron 1
        ctx_a = row['seq_ExonA'][-50:] + row['seq_Intron1'][:50]
        
        # Ctx B: Last 50 Intron 2 + First 50 Exon B
        ctx_b = row['seq_Intron2'][-50:] + row['seq_ExonB'][:50]
        
        p_exon = row['seq_PseudoExon']
        p_int1 = row['seq_Intron1']
        p_int2 = row['seq_Intron2']
        
        # P_Acc: 150bp Intron1 + 50bp PseudoExon
        # Focus on upstream regulatory elements (Branch Point)
        p_acc_intron = p_int1[-150:] 
        p_acc_exon = p_exon[:50] 
        p_acc = p_acc_intron + p_acc_exon
        p_acc = pad_sequence(p_acc, 200, pad_side='right')
        
        # P_Don: 50bp PseudoExon + 150bp Intron2
        # Focus on downstream drug motif context
        p_don_exon = p_exon[-50:] 
        p_don_intron = p_int2[:150]
        p_don = p_don_exon + p_don_intron
        p_don = pad_sequence(p_don, 200, pad_side='left') 
        
        label = float(row['dPSI'])
        
        if len(ctx_a)!=100 or len(ctx_b)!=100: return None
...
def generate_adversarial_decoys(row, target_count=DECOY_RATIO):
    ...
    try:
        ...
        if len(source_seq) < 600: return []
        
        for _ in range(50):
            if len(decoys) >= target_count: break
            don_idx = random.randint(300, len(source_seq) - 200)
            ...
            # Construct Decoy (Matching new window logic)
            fake_intron_before = source_seq[acc_idx-150 : acc_idx]
            fake_exon_start = source_seq[acc_idx : acc_idx+50]
            
            fake_exon_end = source_seq[don_idx-50 : don_idx]
            fake_intron_after = source_seq[don_idx : don_idx+150]
            
            p_acc = fake_intron_before + fake_exon_start
            p_acc = pad_sequence(p_acc, 200, pad_side='right')
            
            p_don = fake_exon_end + fake_intron_after
            p_don = pad_sequence(p_don, 200, pad_side='left')
...
def smart_balance_dataset(data_arrays):
    ...
    return (X_a[final_indices], X_b[final_indices], X_acc[final_indices], X_don[final_indices], y[final_indices])
...
def main():
    ...
    print(f"Generating Deep Context Dataset...")
    ...
    arrays = (np.array(data_a), np.array(data_b), np.array(data_p_acc), np.array(data_p_don), np.array(data_y), np.array(data_is_decoy))
    ...
    # CHANGE: Huber Loss for robustness against outliers
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1} Huber Loss: {total_loss/len(train_loader):.5f}")
