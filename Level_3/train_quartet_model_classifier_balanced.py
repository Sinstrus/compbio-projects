...
def generate_adversarial_decoys(row, target_count=DECOY_RATIO):
    ...
    try:
        ...
        for _ in range(50):
            ...
            decoys.append((ctx_a, ctx_b, p_acc, p_don, 0))
...
def smart_balance_dataset(data_arrays):
    """
    Filters the dataset to ensure High Quality Learning.
    1. Keep ALL Positives (Responders)
    2. Keep ALL Hard Decoys
    3. Downsample Real Negatives to match sum of above.
    """
    ...
    idx_real_neg = np.where((y == 0) & (is_decoy == 0))[0]
    
    n_pos = len(idx_pos)
    n_decoy = len(idx_decoy)
    n_real_neg = len(idx_real_neg)
    
    print("\n--- Smart Balancing ---")
    print(f"  Positives: {n_pos}")
    print(f"  Hard Decoys: {n_decoy}")
    print(f"  Easy Negatives: {n_real_neg}")
    
    # Determine target count for Real Negatives
    # We want a mix. Let's take (Pos + Decoys) * 1.5 to keep some easy negatives context
    target_neg = int((n_pos + n_decoy) * 1.5)
    
    if target_neg < n_real_neg:
        print(f"  -> Downsampling Easy Negatives to {target_neg}...")
        idx_real_neg = np.random.choice(idx_real_neg, target_neg, replace=False)
    else:
        print("  -> Keeping all Easy Negatives (Dataset is small).")
        
    # Combine
    final_indices = np.concatenate([idx_pos, idx_decoy, idx_real_neg])
...
def main():
    ...
    print(f"Generating Dataset (Targeting {DECOY_RATIO} decoys per real sample)...")
    ...
    # BALANCE THE DATA BEFORE SPLITTING
